        ...
        if (!author) {
          await UserModel.findById(id)
            .then(async (user) => {
              if (user) {
                ...
                if (!author) {
                  return res
                    .status(404)
                    .send({ message: `User is not an Author` });
                }
            ...
            .catch((error) => {
              return res
                .status(400)
                .send({ message: `Error get author '${id}': ${error}` });
            });
        } else {
          return res.status(200).send(author);
        }
...
async function postAuthor(req: Request, res: Response) {
  try {
    ...
    const user: User | null = await UserModel.findById(userId).then((user) => {
      if (!user) return null;
      return user;
    });
      ...
      .then(async (author) => {
        if (author) {
          ...
          author.books.forEach(async (element) => {
            await BookModel.findByIdAndUpdate(element._id, {
              writer: anonymous,
            });
          });
