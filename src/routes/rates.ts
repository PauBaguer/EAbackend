...
async function getRates(req: Request, res: Response): Promise<void> {
  try {
    const allRates = await RateModel.find().populate(
      "rating",
      "userId",
      "rate"
    );
...
async function getBookRate(req: Request, res: Response): Promise<void> {
  try {
    const rateFound = await RateModel.findOne({
      bookId: req.params.bookId,
    }).populate("rating", "userId", "rate");
    if (rateFound == null) {
...
async function addRate(req: Request, res: Response): Promise<void> {
  try {
    const { bookId, totalRate, rating } = req.body;
    ...
    await RateModel.findOneAndUpdate(
      ...
      .then((resRate) => {
        if (!resRate) {
          return res
            .status(404)
            .send({ message: "Error add userRate to book." });
        } else {
          res.status(200).send({ message: "Updated!" });
        }
      })
      .catch((error) => {
        return res.status(400).send({ message: `Error rate a book ${error}` });
      });
  } catch (e) {
    res.status(500).send({ message: `Server error: ${e}` });
  }
